...
- **NVIDIA PipeWire capture needs concrete EGL DMA-BUF modifiers.** Query `eglQueryDmaBufFormatsEXT`/`eglQueryDmaBufModifiersEXT`, advertise the concrete modifiers with `DRM_FORMAT_MOD_INVALID` last as a GBM allocation fallback, and acknowledge the transition from the initial implicit format to the fixated DMA-BUF format. Advertising only `INVALID` made Hyprland settle on MemFd capture and forced a GPU→CPU→GPU round trip.
